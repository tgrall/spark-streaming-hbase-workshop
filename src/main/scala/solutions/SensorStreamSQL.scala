...
object SensorStreamSQL {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setAppName("HBaseTest")
      .set("spark.cleaner.ttl", "60").set("spark.files.overwrite", "true")
    val sc = new SparkContext(sparkConf)

    val sqlContext = new SQLContext(sc)
    import sqlContext.implicits._

    val pumpRDD = sc.textFile("/user/user01/sparkstreaminglab/data/sensorvendor.csv").map(parsePumpInfo)
    println("pumpRDD take 1")
    pumpRDD.take(1).foreach(println)
    val maintRDD = sc.textFile("/user/user01/sparkstreaminglab/data/sensormaint.csv").map(parseMaint)
